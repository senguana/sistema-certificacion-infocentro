...
<div class="table-responsive" id="tablaRepre">
	<table id="basic-datatables" class="display table table-striped table-hover"  >
		...
		<tbody>
			...
			$consulta = "SELECT *  FROM alumno_basica a INNER JOIN institucion i ON a.institucion_id = i.id_institucion INNER JOIN grado g ON a.grado_id = g.id_grado WHERE id_grado >= 10 AND estado =1"; 
			...
			while ($result = $query_listar->fetch(PDO::FETCH_OBJ)) {?>
				...
				<td>
					<div class="form-button-action">
						...
						<button type="button" data-toggle="modal"  data-target="#deleteAlumnoBasicaModal" data-id="<?php echo $result->id_alumno_s; ?>" title="" class="btn btn-link btn-danger"   data-original-title="Remove"><i class="fa fa-trash"></i>
							</button>
					</div>
				</td>									
			</tr>
