<?php 
	...
	if (isset($_POST['registrarse'])) {
		...
		if ($crud->buscarUsuario($_POST['username'])) {
			$crud->insertar($usuario);
			header('Location: index.php');
		}else{
			header('Location: ./vistas/login/error.php?mensaje=El nombre de usuario ya existe');
		}		
		
	}elseif (isset($_POST['entrar'])) { //verifica si la variable entrar está definida
		$usuario=$crud->obtenerUsuario($_POST['username'],$_POST['password']);
		...
		if ($usuario->getId()!=NULL) {
			...
			header('location: ./../vistas/login/error.php?mensaje=Tus nombre de usuario o clave son incorrectos'); // cuando los datos son incorrectos envia a la página de error
		}
